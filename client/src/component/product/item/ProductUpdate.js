...
const ProductUpdate = () => {
    ...
    const [contVal, setContVal] = useState(true);
    const [priceVal, setPriceVal] = useState(true);
    const [quantityVal, setQuantityVal] = useState(true);
    ...
    const sendRequest = async () => {
        ...
        setPostName(rd.proname);
        setPostCont(rd.procont);
        setPostPreview(rd.proimg);
        setPostFile(rd.proimg);
        setPostCar(rd.proca);
        if (rd.proca2 !== null) {
            setPostCar2(rd.proca2)
        }
        setPostCar2(rd.proca2);
        setPostPrice(rd.price);
    ...
    const submitClickHandler = (e) => {
        ...
        if (nameVal && priceVal && contVal && quantityVal) {
              axios({
            url: `http://localhost:8080/proupdate/${id}`,
            method: 'put',
            data: postData
    ...
    return (
        <div className={classes.cont}>
            <div className={classes.postname}>상품 수정</div>
